...
Este projeto já está preparado para ser publicado na Vercel usando **WSGI** por meio do arquivo `api/index.py`.

### 1. Pré-requisitos
- Node.js + Vercel CLI (`npm i -g vercel`)
- Conta Vercel configurada (`vercel login`)
- Binário estático do FFmpeg incluído no repositório (ex.: `bin/ffmpeg`) ou fornecido via variável de ambiente

> Consulte `INSTALAR_FFMPEG.md` para saber como gerar ou baixar o binário.

### 2. Configurar a variável `FFMPEG_PATH`
Na Vercel, informe o caminho do binário para que o runtime encontre o executável:
```bash
vercel env add FFMPEG_PATH ./bin/ffmpeg
```
